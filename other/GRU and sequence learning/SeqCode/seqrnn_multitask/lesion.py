...
import numpy as np
#os.chdir('../holmes_m')
from netools import wh_loading
...
def lesion_rnn(rnn_model,lesion_method,anlys_file):
    ...
    numNeuron_con = 0
    model = []
#    lesion_prob = '-'+anlys_file.split('/')[3]
...
def lesion_each(name,lesion_method,rnn_model, neurons,numNeuron_les_con):
    ...
    if 'rnn' in lesion_method:
        model[ name + '_rnn_les'] = set_zeroweight_rnn(rnn_model, neurons)
    return model

def set_zeroweight_rnn(rnn_model, pos_neg):
    numNeuron = rnn_model.decoder.weight.shape[1]
    model_les = copy.deepcopy(rnn_model)
    for i in pos_neg:
        for ii in range(3): # for resetgate/inputgate/ newgate 
            model_les.rnn.weight_hh_l0[int(ii*numNeuron+i),:] = 0
            model_les.rnn.weight_ih_l0[int(ii*numNeuron+i),:] = 0
            model_les.rnn.bias_hh_l0[int(ii*numNeuron+i)] = 0
            model_les.rnn.bias_ih_l0[int(ii*numNeuron+i)] = 0
        model_les.rnn.weight_hh_l0[:,int(i)] = 0
