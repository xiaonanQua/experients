...
# 使用大于两块的gpu
def train_and_valid_gpus(net, criterion, optimizer, train_loader, valid_loader, cfg,
                     is_lr_adjust=True, is_lr_warmup=False):
    ...
    def _train(train_loader, num_step):
        ...
        for index, data in enumerate(train_loader, start=0):
            # 获取每批次的训练数据、并将训练数据放入GPU中
            images, labels = data
	    images = DataParallel(images, device_ids=[0, 1])
	    labels = DataParallel(labels, device_ids=[0, 1])
            # images = images.to(cfg.device)
            # labels = labels.to(cfg.device)
    ...
    for epoch in range(cfg.epochs):
        ...
        if valid_acc > best_acc:
            # 更新最好精度、保存最好的模型参数
            best_acc = valid_acc
            torch.save(net.module.state_dict(), cfg.checkpoints)
            print('  epoch:{}, update model...'.format(epoch))
        print()
